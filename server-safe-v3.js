...
const STATIC_DIR = process.env.STATIC_DIR || path.join(process.cwd(), 'output');
...
app.get('*', (req, res) => {
  const file = req.path === '/' ? 'sfl-mining-calculator-safe-v2.html' : req.path.slice(1);
...
app.listen(PORT, () => {
  console.log(`Safe SFL calculator v2 on http://127.0.0.1:${PORT}`);
